  ...
  def handleArgs(args: Array[String]): RunningArgument= {
    ...
    if(args(11).toBoolean.equals(false)){
      runningArgument = RunningArgument(args(0), args(1), args(2), args(3), args(4), args(5),
        args(6).toInt, args(7).toInt, args(8).toInt, args(9).toInt, args(10).toInt, args(11).toBoolean, null, null)
    }
    else {
      runningArgument = RunningArgument(args(0), args(1), args(2), args(3), args(4), args(5),
        args(6).toInt, args(7).toInt, args(8).toInt, args(9).toInt, args(10).toInt,  args(11).toBoolean, args(12), args(13))
    }
